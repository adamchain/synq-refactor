...
function spawnBuffered(command, args, { debug, ...options } = {}) {
    const childProcess = cross_spawn_1.default.spawn(command, args, options);
...
exports.spawnBuffered = spawnBuffered;
function spawnBufferedSync(command, args, { debug, ...options } = {}) {
    const result = cross_spawn_1.default.sync(command, args, options);
    if (debug) {
...
exports.spawnBufferedSync = spawnBufferedSync;
function execBuffered(command, { debug, windowsHide, ...options } = {}) {
    return toResultPromise(new Promise((resolve) => {
        child_process_1.exec(command, { ...options, windowsHide: windowsHide !== false, encoding: 'buffer' }, (err, stdout, stderr) => {
...
function execBufferedSync(command, { debug, windowsHide, ...options } = {}) {
    try {
...
exports.execBufferedSync = execBufferedSync;
