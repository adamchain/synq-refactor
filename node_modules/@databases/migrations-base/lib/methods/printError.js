...
function printError(error, {
  ...
  switch (error.code) {
    case 'database_version':
      console.error(`${packageName} does not support ${databaseName} version ${chalk.cyan(error.actualVersion)} upgrade to at least version ${chalk.cyan(error.minimumVersion)}.`);
      break;

    case 'database_uses_newer_version':
      console.error(`This database has been migrated using ${packageName} version ${chalk.cyan(error.databaseVersion)}.`);
      console.error(`Your local copy of ${packageName} is version ${chalk.cyan(error.packageVersion)}.`);
      console.error(`Attempting to apply migrations with an outdated copy of ${packageName} could lead to errors.`);
      console.error(`Please update your version of ${packageName}`);
      break;

    case 'database_uses_older_version':
      console.error(`This database has not been migrated using ${packageName} version ${chalk.cyan(error.packageVersion)} before.`);
      console.error(`Version updates cannot be automaticaly applied when --dry-run is enabled.`);
      break;

    case 'migration_filenames':
      console.error(`Migration filenames should start with an integer then a "-". For example:`);
      console.error(``);
      ...
      if (error.files.length === 1) {
        console.error(`${chalk.cyan(error.files[0])} is not a valid migration filename.`);
      } else {
    ...
    case 'migration_has_no_export':
      ...
      console.error(`To be used as a migration it must either have a default export, or a named export called "applyMigration".`);
      break;

    case 'concurrent_migrations':
      console.error(`Another migrations script was run on this database concurrently.`);
    ...
    case 'migration_file_missing':
      ...
      console.error(``);
      console.error(`  ${cliName} restore-from-db -m ${error.appliedMigration.index} <MIGRATIONS_DIRECTORY>`);
      console.error(``);
      console.error(`To ignore the missing file, run:`);
      console.error(``);
      console.error(`  ${cliName} ignore-error -m ${error.appliedMigration.index} -e migration_file_missing <MIGRATIONS_DIRECTORY>`);
      console.error(``);
    ...
    case 'migration_file_edited':
      ...
      console.error(``);
      console.error(`  ${cliName} mark-unapplied -m ${error.appliedMigration.index} <MIGRATIONS_DIRECTORY>`);
      console.error(``);
      console.error(`Then re-run: ${cliName} apply  <MIGRATIONS_DIRECTORY>`);
      console.error(``);
      console.error(`To restore the original file, delete the current one then run:`);
      console.error(``);
      console.error(`  ${cliName} restore-from-db -m ${error.appliedMigration.index} <MIGRATIONS_DIRECTORY>`);
      console.error(``);
      console.error(`To ignore the discrepancy file, run:`);
      console.error(``);
      console.error(`  ${cliName} ignore-error -m ${error.appliedMigration.index} -e migration_file_edited <MIGRATIONS_DIRECTORY>`);
      console.error(``);
    ...
    case 'migration_order_change':
      ...
      for (const migration of error.appliedMigrations) {
        console.error(` - ${chalk.red(migration.name)} applied at ${migration.applied_at.toISOString()}`);
      }

      console.error(``);
      console.error(`To apply the migrations even though they are out of order, run:`);
      console.error(``);
      console.error(`  ${cliName} apply -e migration_order_change <MIGRATIONS_DIRECTORY>`);
      console.error(``);
      break;
    ...
    default:
      void assert_never_1.default(error, true);
