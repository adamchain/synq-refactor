...
const pg_1 = require("@databases/pg"); // We are using sucrase to compile TypeScript on the fly, and each of these tests
...
function output(proc) {
  ...
  return [`stdout:`, (_a = proc.stdout) === null || _a === void 0 ? void 0 : _a.toString('utf8').replace(/^/gm, '  '), `stderr:`, (_b = proc.stderr) === null || _b === void 0 ? void 0 : _b.toString('utf8').replace(/^/gm, '  ')].join('\n');
}
...
function run(params, options = {}) {
  ...
  // );
  const proc = child_process_1.spawnSync('node', ['../lib/cli', ...params], {
    cwd: `${__dirname}/..`,
    env,
    timeout: 5000,
    ...options
...
test('migrations applied', async () => {
  expect(await db.query(pg_1.sql`SELECT * FROM users`)).toEqual([{
    id: BigInt(1),
